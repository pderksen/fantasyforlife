...
`output/rules.html`, generated by `generateRulesHtml` in `src/html.ts`, served at `/rules`. Root
level, so it takes the index's chrome (`base: ""`, no `fullBleed`) and the same 1080px measure,
and a flat file for the same `file://` preview reason as the History, Prize and Tiers pages.

The content is `src/rules.ts`, its own module rather than more of `league-info.ts`. That is the
call `tiers.ts` already made: a full rules set is about four thousand words, more than the whole
of `league-info.ts` holds today, and unlike everything in that file it turns over completely each
August instead of accumulating a season at a time.

**One page with anchors, not a page per section.** The same reasoning the League History page
used. A rules set is shorter than one roster table, so length is not the problem it would need to
be; it is replaced wholesale each year rather than growing, so the file count would never settle;
...
visible. The title is both the heading and its contents entry, from the same string, so those two
...
While `RULES_SECTIONS` is empty the contents list and the sections are replaced by a single card
naming the season and pointing at the newest set in the archive below. It is derived from that
emptiness, so writing the rules retires it with no second edit.

It names the newest archived season rather than saying "coming soon" and stopping, because a
reader who came for the rules can be sent to the most recent published set in the same sentence,
and that set is one click down the same page. This is why the page went live in the site nav
before the current season's rules existed: seventeen years of rules documents that are reachable
from nowhere else on the site are worth the nav item on their own.

### Past Years' Rules

Year tiles in a card, typed like the History page's Old League Sites section, which is the same
object seen twice: a row of years, each opening a document that is not this page. Tiles rather
than a table for the reason that section gives, one link per row being four words in a
five-column frame.

**The two kinds of entry are labelled groups, not one mixed row**, because they behave differently
under a click. A Google Doc leaves the site, so it opens in a new tab and carries the `↗` mark; a
frozen season is a page here and opens in place. Only a group with entries renders, so the card is
a single group today and says nothing about a form of link it does not yet hold. Each group after
the first takes a `border-t` above it, the same shape the Past Leagues card uses for its Sleeper
and MyFantasyLeague halves.

**The archive is two lists that move in opposite directions.** `RULES_DOC_LINKS` is closed and
2025 is the last entry it will ever take, since every season from 2026 is written and frozen here.
`RULES_PAGE_SEASONS` is empty today and grows by one each August. Keeping them apart rather than
merging into one hand-typed list of URLs is what makes each yearly edit a single line: a frozen
season's filename is a convention `rulesPageFile()` builds, so a season cannot be listed under a
name the site does not actually serve.

**The missing years are derived.** No rules document survives for 2006, 2007 or 2010, and the note
under the card reads that gap off the league's own season range against the links that exist,
rather than naming three years in prose. Finding a 2010 document later is one entry in
`RULES_DOC_LINKS`, and the note drops the year on its own. The years themselves are simply absent
from the tiles, per the same rule the Scoring Records tables follow: a record nobody captured is
an absent row and not a blank one.

**A published doc is proved by its body, never by its status code.** Three separate traps sit in
front of this, and each returns a confident wrong answer rather than an error. A bare `curl` gets
a 401 on a document that is published perfectly well, because Google rejects its default user
agent, so a plain request reads as unpublished and is not evidence of anything. Supply a browser
agent and an *unpublished* doc then answers 200, serving a sign-in page in place of the document,
so a status check passes on exactly the link that is broken. And Drive's permissions API cannot
see publish-to-web state at all, reporting owner-only for all seventeen of these regardless.

What works is `curl -sL -A "Mozilla/5.0"` followed by reading what came back: a real document
carries a `<title>` and runs about 215KB, while the sign-in wall carries no title, runs about
9KB, and says "You must sign in to access this content". All seventeen were swept this way on
2026-08-22. 2011 failed the first sweep, was published that day, and passes now. 2008 through
