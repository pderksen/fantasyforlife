...
- `output/history.html` — League History page, served at `/history`. Rewritten by every run. Sub-nav tab bar (`HISTORY_SECTIONS` in `html.ts`, one tab per section of the page) → newest season's honor cards → the all-time table → earlier seasons → "Back to top". Anchors within the one page, not a file per season
