...
## Index Page UI
The home page (`output/index.html`, generated by `generateIndexHtml` in `src/html.ts`) uses a clean, all-white minimal (Apple-esque) design with zero dependencies — pure HTML/CSS inline.

### Layout
- Centered container (max-width 680px), generous whitespace (80px top padding)
- System font stack (`-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial`)
- League name large/bold/centered with "est. 2006" byline in muted gray below

### Sections
1. **"Current Tiers by Season"** — Season rows with year on left, pill-style button links on right. Seasons sorted most-recent first. Only shows links for snapshots that exist on disk.
2. **Traded Picks** — Clean borderless table with subtle row separators. Only shown if future traded picks exist. Built inline (not using the shared `tradedPicksSection` helper used by roster pages).
3. **Archive link** — "Seasons 2006–2024 ↗" linking to the Google Sheet. Opens in new tab. 18px blue text, underline on hover. No heading or wrapper text.

### Throwback Year
Seasons with snapshots but no pre-draft page (e.g., 2025) show a "Throwback Year" label to the left of the pill buttons. Small amber text (`#92400e`, 12px), not italic, not a button/link. Positioned with `margin-right: auto` in flexbox. This is a rare case (once every 5–10 years). Future seasons (2026+) will have pre-draft and won't show this label.

### Pill buttons
White background, 1px gray border (`#d2d2d7`), 8px border-radius, subtle shadow. On hover: blue background (`#0071e3`), white text, blue shadow. Labeled "Pre-Draft", "Post-Draft", "End-of-Season" (derived from `SNAPSHOT_TYPE_LABELS` with " Rosters" stripped).
