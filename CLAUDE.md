...
**CSV Output**: every roster page has a `<season>-rosters-<type>.csv` twin beside it, written by the same run through `writeRosterOutputs()` in `index.ts` — the page links its own CSV, so one is never written without the other. Nothing reads these back; they exist for download. See the CSV Export section.
...
- `src/snapshot.ts` — Snapshot capture/save/load, path helpers, draft round lookup, traded picks resolution + display filters (`picksForDraft()`, `picksAwaitingDraft()`, `newestDataSeason()`), trade log resolution/save/load (`resolveTrades()`, `saveTrades()`, `loadTrades()` — `loadTrades()` has no caller since the page was dropped; it is the read half of the archive), page discovery + nav (`discoverPages()`, `pageFileName()`, `csvFileName()`), pre-draft overwrite guard (`preDraftWindowClosed()`, `SnapshotGuardError`). `OWNER_NAME_OVERRIDES`: `ClovisJets` → `Clovis Jets`
- `src/roster-grid.ts` — the roster table as data, one step short of markup: `buildRosterGrid()` picks the layout (sequential, tiered, post-draft-by-round), sorts the owner columns, and returns `GridRow[]`. Both renderers build from it, so the page and its CSV cannot drift. `DraftRoundLookup` lives here
...
- `src/csv.ts` — CSV generation from the same grid (`generateCsv()`, `writeCsv()`)
...
- `output/<season>/<season>-rosters-<type>.csv` — CSV twin of each roster page, rewritten with it
...
- **CSV pill**: nav's last item, pushed right with `ml-auto` — a download icon plus "CSV", linking `<season>-rosters-<type>.csv` as a sibling with the `download` attribute. `PILL_EXPORT` shares `PILL_BOX` and `PILL_LINK_COLORS` with the other pills but swaps `inline-block` for `inline-flex` rather than adding it: two `display` utilities on one element resolve by stylesheet order, not attribute order, so the loser would be picked silently
...
`output/<season>/<season>-rosters-<type>.csv`, generated by `generateCsv()` in `src/csv.ts` and written by the same call that writes the page. Write-only output: nothing in the codebase reads it back, and the page's own download link is the only consumer.

**The CSV name repeats the season, the page's does not.** A page is read in place, under a URL that already says which year it is; its CSV gets downloaded into a folder alongside every other year's export, where a bare `rosters-pre-draft.csv` identifies nothing. So the two names come from separate helpers in `snapshot.ts` — `pageFileName()` and `csvFileName()` — rather than one stem with two extensions.

**Mirrors the page, not the snapshot.** Both renderers consume `buildRosterGrid()`, so column order, tier placement, keeper-first sorting, and round labels are decided once. Changing a layout rule changes both; rendering a CSV straight off the JSON would have let the two drift apart silently.

- **Grid shape, matching the historical Google Sheet**: owners across the header, players down, cells reading `Last, First TEAM POS`. Post-draft keeps its leading "Round" column ("4a", "4b").
- **Tier bars become rows**: a spreadsheet has no `colspan`, so the label sits in the first column and the rest of the row is blank.
- **Keepers get a trailing ` *`** (`KEEPER_MARK`). The page says it with a yellow cell, and no highlight survives an export, so the flag has to ride in the text.
- **UTF-8 BOM**, or Excel on Windows reads the file as the system codepage and turns the em dashes in tier labels into mojibake. Everything else skips the mark silently.
- **LF line endings, RFC 4180 quoting** (quote on delimiter/quote/newline, doubled quotes). Player names are `Last, First`, so nearly every cell quotes.
- **The page's link comes from `csvFileName()`**, the same helper `getCsvOutputPath()` writes through, so the name can't drift out from under the relative link.
...
## Manual Editing
Snapshot JSON files are human-readable and editable. Regenerate the pages and their CSVs after edits: `npm run dev -- --generate <season> [type]`
