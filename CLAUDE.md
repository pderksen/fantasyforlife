...
## Index Page UI
Generated by `generateIndexHtml` in `src/html.ts`. Light mode, Tailwind CDN + Inter font, centered `max-w-2xl` container.

**Sections**:
1. **"Current Tiers"** — Season rows (year left, chip links right), most-recent first. Archive link ("Seasons 2006–2024 ↗", `text-xl text-blue-600`) appears below the oldest season row, inside this section.
...
**Season chips**: `bg-gray-100 text-gray-700 rounded-lg`, hover `bg-gray-200`. Labels from `SNAPSHOT_TYPE_LABELS` with " Rosters" stripped.

## Roster Page UI
Generated by `generateHtml` in `src/html.ts`. Light mode, `bg-gray-50` body.
...
**Traded Picks**: `<h2>` heading + narrow table (`inline-block` wrapper) at end-of-season, or "None" for pre/post-draft.
