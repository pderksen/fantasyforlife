...
Abbreviating is fine where a full slate of names would not fit and the reader already has the context: `PRIZE_WINNERS` in `league-info.ts` is the standing example, using bare city words ("Vancouver", "Visalia", "Sanger"). The League History table is the second, and it abbreviates **at render** (`shortenForHistory()` in `html.ts`) so `LEAGUE_HISTORY` still stores full names: a tie drops both teams to city words via `TEAM_CITIES`, and "South Town Freedom Fighters" renders "South Town FF" there and only there. Full names are the default everywhere a team is identified on its own — roster column headers, the draft order card, honor cards, traded-pick owners.
