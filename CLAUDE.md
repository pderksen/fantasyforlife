...
- **HTML row labels**: Post-draft tables have a "Round" column showing draft round numbers. Pre-draft and end-of-season tables have no row-number column.
...
- **Owner header row**: dark gray (`bg-gray-800`)
...
## Draft Order
The upcoming season's draft order is displayed on the index page. Configured manually in `src/tiers.ts` alongside tier configs.

- **Config**: `DRAFT_ORDERS` map keyed by season string (e.g., `"2026"`). Value is an array of owner names in pick order (index 0 = pick 1).
- **Display**: `getLatestDraftOrder()` returns the most recent season's order. Only one season is shown at a time on the index page.
- **Adding a new season**: Add a new entry to `DRAFT_ORDERS` in `src/tiers.ts`. The previous season's entry can remain (only the latest is displayed).
...
## Index Page UI
The home page (`output/index.html`, generated by `generateIndexHtml` in `src/html.ts`) uses Tailwind CSS (CDN) with Inter font. Light mode, minimal design.

### Layout
- Centered container (`max-w-2xl mx-auto px-6 py-20`)
- Inter font via Google Fonts CDN, configured as `fontFamily.sans` in Tailwind config
- League name large/bold/centered (`text-4xl font-bold tracking-tight`) with "est. 2006" byline in `text-gray-400`
- Section headings: `text-sm font-semibold uppercase tracking-wide text-gray-500`
...
### Throwback Year
Seasons with snapshots but no pre-draft page (e.g., 2025) show a "Throwback Year" label to the left of the chip links. `text-xs text-amber-700`, not italic, not a button/link. Positioned with `mr-auto` in flexbox. This is a rare case (once every 5–10 years). Future seasons (2026+) will have pre-draft and won't show this label.

### Season link chips
Muted style: `bg-gray-100 text-gray-700 rounded-lg`, darkens to `bg-gray-200` on hover. Labeled "Pre-Draft", "Post-Draft", "End-of-Season" (derived from `SNAPSHOT_TYPE_LABELS` with " Rosters" stripped).

## Roster Page UI
Roster pages (`output/<season>/rosters-<type>.html`, generated by `generateHtml` in `src/html.ts`) use Tailwind CSS (CDN) with Inter font, same as index page. Light mode with `bg-gray-50` body.

### Page header
- `<h1>` with season year + snapshot type label (e.g., "2025 Post-Draft Rosters") in `text-3xl font-bold tracking-tight text-gray-900`
- League name below as `text-sm text-gray-500`

### Nav bar
- Chip-style links matching the index page's season link style: `bg-gray-100 text-gray-700 rounded-lg`, darkens to `bg-gray-200` on hover
- "Home" link + same-season snapshot links in a flex row with `gap-2` (no border/shadow wrapper)
- Active page: `bg-gray-200 text-gray-900` (non-clickable `<span>`). Other links: standard muted chip style
- All items `text-sm font-medium` with `px-3.5 py-1.5` padding
...
- Timestamp formatted in Pacific time (PT) via `formatPacificTime()` helper using `America/Los_Angeles` timezone
