...
- **A champion is named twice, in `SEASON_HONORS` and in `LEAGUE_HISTORY`, and nothing reconciles them.** The table is deliberately not derived from the cards: honors are a free-form set per season, the table is a fixed four-column spine, and matching a card by its label string would break the day a label is reworded. The price is that a reworded winner has to be changed in both. `LEAGUE_HISTORY` is stored oldest-first and rendered newest-first; blank cells are expected (2006–2023 aren't recorded anywhere yet) and render as an em dash with a note under the table.
