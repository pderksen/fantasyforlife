...
- `output/history.html` — League History page, served at `/history`. Rewritten by every run. Sub-nav pills (all-time table, then each season newest-first) → newest season's honor cards → the all-time table → earlier seasons. Anchors within the one page, not a file per season
...
- **The League History table is styled off the home page's draft order card, not off the traded-picks tables.** Same `CARD` + `bg-shell` header strip + 15px rows on `border-t` hairlines (`HIST_TH` / `HIST_TD` in [html.ts](src/html.ts)), so the two lists read as a pair. Every cell is `whitespace-nowrap` — no row ever wraps to a second line, so a name too long for the measure makes the card scroll sideways instead. That is the budget `shortenForHistory()` protects; a longer name or a seventh column spends it. **Below `sm` the table is replaced, not reflowed**: `historyStackHtml()` renders one labeled block per season and a Tailwind visibility pair (`sm:hidden` / `hidden sm:block`) picks which is shown, so both are always in the HTML. Six columns of names fit no phone at any font size, which is why the wide layout is free to keep full names. Both layouts build from `HISTORY_COLUMNS`, so a column is defined once; the stack drops every name to a city word and omits a field it has no name for rather than dashing it.
- **A champion is named twice, in `SEASON_HONORS` and in `LEAGUE_HISTORY`, and nothing reconciles them.** The table is deliberately not derived from the cards: honors are a free-form set per season, the table is a fixed six-column spine (Season, Champion, Runner-Up, Toilet Bowl, Total Points, Best Record), and matching a card by its label string would break the day a label is reworded. The price is that a reworded winner has to be changed in both. The last two columns name a **team only** — no point total, no win-loss record; those stay on the honor cards, and they name it as a **city word at every width** (`city` in `HISTORY_COLUMNS`), which is what keeps the six-column measure affordable. `LEAGUE_HISTORY` is stored oldest-first and rendered newest-first. All 20 seasons are now filled in, 2006–2023 from the FFL Champions section of the private `FFL History & Records` Google Doc — that section records the three bracket finishes only, so every pre-2025 row leaves Total Points and Best Record blank (em dash on the wide layout, omitted on the stack). Folded teams (Winnemucca, Chico, Canton, Collet, Biola) live only in those rows and join on nothing, but they do need a `TEAM_CITIES` entry or they render at full length on mobile. The "still being compiled" note under the table is derived from the oldest row, so it took itself out.
